...
const getAllSellers = async (req, res) => {
  ...
  try {
    ...
    const result = await sellerModel.selectAllSellers(
      querySearch,
      querySortBy,
      querySort,
      queryLimit,
...
const registerSeller = (req, res) => {
  ...
  sellerModel
    ...
    .then((result) => {
      // Display the result
      succesTemplate.responseTemplate(res, 201, result.rows, "seller created");
    })
    .catch((err) => {
      console.log(err);
      res.send(err.detail);
